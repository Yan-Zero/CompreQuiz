    ...
    for filename in os.listdir(path):
        ...
        with open(os.path.join(path, filename), 'r', encoding='utf-8') as file:
            ...
            for question_data in _question_data:
                ...
                question_data['weight'] = weights.get(hashlib.sha1(question_data['question'].encode()).hexdigest(), 1.0)
...
def save_weights(questions: list[Question], path: str):
    weights = {}
    for question in questions:
        weights[hashlib.sha1(question.question.encode()).hexdigest()] = question.weight
