...
class CcWord:
  def __init__(self, char: str, meanings: list[str], examples: list[dict[str, tuple[int, int]]], weights: list[float]):
    self.char = char
  ...
  def __init__(self, poems: dict, weights: dict[str, list[float]], word_dict: dict = None, path: str = words_path):
    def find_sentence_in_poems(sentence: str, poem: Poem) -> tuple[int, int]:
      for idx, line in enumerate(poem.Content):
          if sentence == line:
              return idx, 0
      raise ValueError(f'Cannot find sentence "{sentence}" in poem "{poem.Title}"')
    def load_word_file(file_path: str):
      words = []
      _words = load_yaml(file_path)
      for word in _words:
        for i, ex in enumerate(word['examples']):
          word['examples'][i] = {k: [eval(x) if x[0] == "(" else find_sentence_in_poems(x, poems[k]) for x in v] for k, v in ex.items()}
          word['weights'] = weights.get(word['char'], [1.0] * len(word['examples']))
  ...
  def load_question_file(file_path: str, poems: dict, weights: dict):
    ...
    for qd in _question_data:
      ...
      if 'answers_str' in qd:
        qd['answers'].extend([[]] * (len(qd['answers_str']) - len(qd['answers'])))
...
def gene_word_question(q: CompreQuestion, index: int, m: dict) -> list[WordQuestion]:
  ...
  count = min(sum(1 for _ in range(len(t)) if random.uniform(0, w * 5) < w), len(t))
